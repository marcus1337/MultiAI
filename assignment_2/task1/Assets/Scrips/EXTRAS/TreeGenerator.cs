...
public class TreeGenerator
{
        ...
        while (done)
        {
            foreach (Robot robot in robots)
            {
                if (robot.noMoreMoves)
                    continue;
                var moves = getPossibleMoves(robot.subtree.Last.Value, robot.ID);
    ...
    {
        int x1, x2, y1, y2;
        x1 = n1.mapX; x2 = n2.mapX; y1 = n1.mapY; y2 = n2.mapY;
    ...
    private int Manhattan(Node n1, Node n2) { return (int)(Math.Abs((n1.position.x - n2.position.x)) + Math.Abs(n1.position.y - n2.position.y)); }

    List<Node> getPossibleMoves(Node nowNode, int cur_robotID)
    {
    ...
    {
        int bestDistance = Int32.MinValue;
        Node chosenNode = null;
        foreach (Node n in movesPossible)
        {
            int dist = MinDist(n, cur_robotID);
    ...
    private int MinDist(Node potential_move, int cur_robotID)
    {
