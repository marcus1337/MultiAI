...
public class ANode : IHasNeighbours<ANode>
{
    ...
    private static readonly int extraCost = 150;
    private static readonly int turnCost = 3;
    ...
    public ANode UpLeft
    {
        get
        {
            ...
            if (carDir == DOWN || carDir == RIGHT || carDir == DOWNRIGHT)
                return grid.GetNodeStar(this, x - 1, y - 1, cost + extraCost, UPLEFT, maxTurns);
            if (carDir != UPLEFT)
                return grid.GetNodeStar(this, x - 1, y - 1, cost + turnCost, UPLEFT, maxTurns);
            return grid.GetNodeStar(this, x - 1, y - 1, cost, UPLEFT, turns - 1);
        }
    }
    ...
    public ANode UpRight
    {
        get
        {
            ...
            if (carDir == DOWN || carDir == LEFT || carDir == DOWNLEFT)
                return grid.GetNodeStar(this, x + 1, y - 1, cost + extraCost, UPRIGHT, maxTurns);
            if (carDir != UPRIGHT)
                return grid.GetNodeStar(this, x + 1, y - 1, cost + turnCost, UPRIGHT, maxTurns);
            return grid.GetNodeStar(this, x + 1, y - 1, cost, UPRIGHT, turns - 1);
        }
    }
    ...
    public ANode DownRight
    {
        get
        {
            ...
            if (carDir == UP || carDir == LEFT || carDir == UPLEFT)
                return grid.GetNodeStar(this, x + 1, y + 1, cost + extraCost, DOWNRIGHT, maxTurns);
            if (carDir != DOWNRIGHT)
                return grid.GetNodeStar(this, x + 1, y + 1, cost + turnCost, DOWNRIGHT, maxTurns);
            return grid.GetNodeStar(this, x + 1, y + 1, cost, DOWNRIGHT, turns - 1);
        }
    }
    ...
    public ANode DownLeft
    {
        get
        {
            ...
            if (carDir == UP || carDir == RIGHT || carDir == UPRIGHT)
                return grid.GetNodeStar(this, x - 1, y + 1, cost + extraCost, DOWNLEFT, maxTurns);
            if (carDir != DOWNLEFT)
                return grid.GetNodeStar(this, x - 1, y + 1, cost + turnCost, DOWNLEFT, maxTurns);
            return grid.GetNodeStar(this, x - 1, y + 1, cost, DOWNLEFT, turns - 1);
        }
    }
