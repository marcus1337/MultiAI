...
public class IOstuff
{
    ...
    public void storeTextCosts(string filename, CostGrid grid)
    {
        string path = Application.dataPath + "/" + filename + ".txt";
        width = grid.width;
        height = grid.height;
        ...
        string gridinfo = "width: " + width + " height: " + height + "\n";
        gridinfo += getMazeStrCosts(grid.costMap);
        File.WriteAllText(path, gridinfo);
    }

    private string getMazeStrCosts(CostPoint[,] M)
    {
        ...
        for (int i = 0; i < height; i++)
        {
            ...
            {
                if (M[j, height-1-i].numTurrets == 0)
                {
                    sb.Append("|" +" " + "|");
                }
                else
                    sb.Append("|"+ M[j, height - 1 - i].numTurrets.ToString() + "|");
